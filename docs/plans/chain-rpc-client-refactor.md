...
>
> **Related document**: `tron-hd-activation-v2.md` (TRON HD activation plan)
...
### Sequencing
**TRX activation MVP is the priority**, as defined in `docs/plans/tron-hd-activation-v2.md` (wallet-only activation via existing ETH routes, correct `T...` addresses, real TRON current block + balance, HD "is used" checks).
...
## 10) Current Code Reality

**TRON RPC integration already landed**, but in a plan-inconsistent shape:
...
**But critical issues exist:**

❌ `ChainRpcClient` abstraction is TRON-special-cased, not chain-agnostic
❌ Call sites use `rpc_client.is_some()` as "is TRON" detector — **will break EVM when `ChainRpcClient::Evm` is added**
❌ Address formatting for TRX still returns `0x...` instead of `T...`
❌ Error modeling doesn't match mm2 patterns

---

## 11) CRITICAL: The `rpc_client.is_some()` Trap

**This is the most dangerous latent bug.** Current code uses `rpc_client.is_some()` to detect TRON:

| Location | Code Pattern | Impact When EVM Uses rpc_client |
|----------|-------------|--------------------------------|
| `eth_rpc.rs::balance()` | `if let Some(ref rpc_client)` → TRON path | EVM balance calls TRON method → `WrongChain` error |
| `eth_hd_wallet.rs::is_address_used()` | `if let Some(ref rpc_client)` → TRON path | EVM HD scanning breaks |
| `eth_hd_wallet.rs::known_address_balance()` | `if rpc_client.is_some()` → skip tokens | **EVM HD balances silently stop including ERC20 tokens** |
| `eth.rs::current_block()` | `if rpc_client present` → use it | EVM returns `NotImplemented` instead of web3 |
...
| PR | Purpose | Status |
|---:|---------|--------|
| **1** | Chain-agnostic read RPC + fix `is_some()` traps | ✅ Done |
| **2** | TRON activation gating (reject token/NFT) | ✅ Done |
| **3** | Address formatting (`T...` everywhere) | 🔄 In Progress |
| **4** | TRX tests + docs updates | 📋 Pending |
...
## 20) PR Summary Table (Updated)

| PR | Purpose | Depends On | MVP? | Blocks EVM? |
|---:|---------|------------|------|-------------|
| **1** | Chain-agnostic read RPC + fix `is_some()` traps | — | ✅ Guardrail | **YES** |
| **2** | TRON activation gating (reject token/NFT) | PR-1 | ✅ | No |
| **3** | Address formatting (`T...` everywhere) | PR-1 | ✅ | No |
| **4** | TRX tests + docs updates | PR-2, PR-3 | Optional | No |
| **4.5** | **ChainBackend composition** (eliminate redundancy) | PR-1, PR-2/3/4 | Post-MVP | Reduces blast radius |
| **5** | EVM scaffolding (RpcLoopSpawner) | PR-1 | Post-MVP | Recommended |
| **6** | EVM backend bootstrap (`web3_instances` → `EvmRpcClient`) | PR-5 | Post-MVP | **YES** |
| **7** | EVM broadcast session abstraction | PR-6 | Post-MVP | **YES** |
| **8** | EVM broadcast reliability (policy + concurrency) | PR-7 | Post-MVP | **YES** |
| **9** | Receipt finality (min_confirmations) | PR-8 | Post-MVP | **YES** |
| **X** | **ChainCoin typed model** (ideal end-state) | PR-6+ | Future | No |
